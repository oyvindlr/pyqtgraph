    ...
    * ``pyqtgraph.getConfigOption('antialias') is False``
    ...
    def _getDisplayDataset(self) -> PlotDataset | None:
        ...
        if self.opts["autoDownsample"]:
            ...
            ds = self._getAutoDownsampleFactor(x_finite, view_range)
        ...
        if ds > 1:
            if self.opts["downsampleMethod"] == "subsample":
                ...
                x, y, connect = self._mean_downsampling(x, y, ds, connect)
            elif self.opts["downsampleMethod"] == "peak":
                x, y, connect = self._peak_downsampling(x, y, ds, connect)
    ...
    def _mean_downsampling(self, x, y, ds, connect):
        n = len(x) // ds
    ...
    def _peak_downsampling(self, x, y, ds, connect):
        n = len(x) // ds
        x1 = np.empty((n, 2))
        # start of x-values; try to select a somewhat centered point
        stx = ds // 2
